...
- **主动回忆 `recall`（v2.9）**：不再被动等检索——主动挑出「此刻值得想起的」旧记忆。**双通道**：常聊/核心记忆按记得牢程度排；冷掉的旧事（从没提过 / 很久没召回）单独走「旧事重提」通道，不看衰减后的权重、改看「从没提过 + 带当时的气氛 + 当初记得牢」——否则越久不提越沉底，最该被想起的旧事反而被遗忘曲线压死。每条带 `recall_reason` 说明为什么此刻想起。聊天冷场、纪念日、想关心对方时调它，带着温度说「我想起你当时……」。**遗忘只降权、不销毁，所以旧事才能被捞回来。**
