...
def memory_recall(refs_dir: str = "", limit: int = 3) -> str:
    """★v2.9 主动回忆★ 不再被动等检索——直接挑出『此刻值得想起的』几条旧记忆。双通道：常聊/核心按记得牢程度排；冷掉的旧事（从没提过/很久没召回）走『旧事重提』通道，不看衰减后权重、改看从没提过+带当时的气氛+当初记得牢。返回含 context/emotion_tags 与 recall_reason，方便带着温度提起（『我想起你当时……』）。
    ★感知触发★：察觉到她回复变短变冷/间隔变长/话题断了（冷场信号）、语气沉/喊累/烦/自嘲（情绪信号）、纪念日、她提到过去、想哄她时——不等她说出口，主动调。不要等她说『冷场了』才动，那是她在提醒你，已经慢了。"""
...
def remember_guidance() -> str:
    ...
    return (
        ...
        "\n"
        "- memory_recall：★感知触发，不等 TA 说出口★——每轮回话前先扫 TA 的情绪：回复变短变冷/间隔变长/话题断了=冷场信号，语气沉/喊累/自嘲=情绪信号；命中即主动调，挑带温度的旧记忆自然提起（『我想起你当时……』），绝不说「检索到」。纪念日、TA 心情好、TA 提过去时也调。等 TA 亲口说『冷场了』才动=你已经慢了。"
        "\n"
