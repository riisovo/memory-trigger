...
# -*- coding: utf-8 -*-
"""
merge_migrate.py —— 双源记忆合并迁移（PATCH v2.6 by Plato）
============================================
用途：agent "接入记忆技能"时，把两套记忆安全并成一套干净初始库：
  源 A（文件，权威）：已有的本地 memory.json（如 Marvis 产物 / 历史沉淀）
...
合并规则（信任优先级固化）：
  ...
  4) 字段校验（防 kuro 式字段错位）：kind 必须 ∈ ALLOWED_KINDS；value 非空且
...
DEFAULT_ALIASES = {
    "那家拉面店": "拉面", "那家拉面": "拉面", "豚骨拉面": "拉面",
    "日式拉面": "拉面", "一兰拉面": "拉面", "味千拉面": "拉面",
    "拉面馆": "拉面", "拉面店": "拉面",
    "波霸奶茶": "奶茶", "珍珠奶茶": "奶茶", "一点点奶茶": "奶茶",
    "冰美式": "咖啡", "美式咖啡": "咖啡", "拿铁": "咖啡",
}
