...
def main():
    ...
    try:
        ...
        r = subprocess.run([py, wp, "search", "拉面", rdir], capture_output=True, text=True)
    ...
    if os.path.isfile(install_sh):
        ...
        check("install.sh 跑 v2.6 双源自检 test_dual_source.py",
              ...
              detail="仅跑 test_ramen.py，v2.6 逻辑装完不验" if 'test_ramen.py' in sh and 'test_dual_source.py' not in sh else "")
    else:
